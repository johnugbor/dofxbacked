...
from .serializers import WalletSerializer,TransactionSerializer,TradeTransactionSerializer
...
class BuyOrder(APIView):
	...
	def post(self, request):
		...
		if success:
			...
			wallet.openbuy(asset_name,asset_volume,useremail,cost,open_price,profit,stop_loss_value,stop_loss_price,symbol,leverage )
...
class SellOrder(APIView):
	...
	def post(self, request):
		...
		if success:
			...
			wallet.opensell(asset_name,asset_volume,useremail,cost,open_price,profit,stop_loss_value,stop_loss_price,symbol,leverage )
...
class ClosePosition(APIView):
	...
	def put(self,request,format=None):
		...
		if success:
			...
			tradedata = TradeTransaction.objects.get(id=pk,user=user)
		...
		if success:
			...
			serializeddata = TradeTransactionSerializer(tradedata, data=request.data)
